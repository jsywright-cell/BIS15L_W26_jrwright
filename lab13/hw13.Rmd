...
Subspecies and female body mass at maturity have the most NAs - in fact, they are all NAs.
...
amniota %>% 
  group_by(class) %>% 
  summarise(n_na_egg_mass_g=sum(is.na(egg_mass_g)))
```

The very large number of missing egg masses makes sense for mammals, as there are only 5 monotremes on earth, but not so much for Aves and Reptilia. The large number of NAs could indicate that any conclusions based on data analysis may be inaccurate because of a lot of missing data. NAs can help determine if a dataset really encompasses all it appears to.
