...
```{r}
p+geom_point(mapping=aes(color=thermoregulation), size=2)
```

We can also map shapes to another categorical variable.
```{r}
p+geom_point(mapping=aes(shape=thermoregulation, color=thermoregulation), size=2)
```

What about just filling in with a color preference or size?
```{r}
p+geom_point(size=3, color="black", alpha=1, shape=6, fill="lemonchiffon4")
```
